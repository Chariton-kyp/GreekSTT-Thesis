...
transcription_response_model = api.model('TranscriptionResponse', {
    ...
    'model': fields.String(description='ASR model used', example='whisper', enum=['whisper', 'wav2vec2']),
    'duration': fields.Float(description='Processing duration in seconds', example=30.2),
    'speed_factor': fields.Float(description='Processing speed vs realtime (e.g., 7.0 = 7x realtime)', example=7.0),
    'wer': fields.Float(description='Word Error Rate (if ground truth available)', example=0.025),
    'cer': fields.Float(description='Character Error Rate (if ground truth available)', example=0.013),
    'accuracy': fields.Float(description='Accuracy percentage', example=97.56),
