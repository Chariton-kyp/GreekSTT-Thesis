"""
WER/CER Calculator for Greek ASR
...
specifically for Greek language with proper handling of:
...
class LevenshteinCalculator:
    """Levenshtein distance calculator with detailed operation tracking."""
    
    @staticmethod
    def calculate(s1: List[str], s2: List[str]) -> int:
        """Calculate Levenshtein distance using space-optimized algorithm."""
        if len(s1) < len(s2):
            return LevenshteinCalculator.calculate(s2, s1)
...
def normalize_greek_text(text: str) -> str:
    ...
    Returns:
        Normalized text
    """
    normalizer = GreekTextNormalizer()
...
class AdvancedWERCalculator:
    ...
    This class provides the same interface as the original for backward compatibility.
    """
