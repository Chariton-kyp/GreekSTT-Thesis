...
if [ -S /var/run/docker.sock ] && is_docker; then
    ...
    if ! groups | grep -q docker; then
        echo "WARNING: Could not add user to docker group, trying alternative approach..."
        # This requires the container to run with sufficient privileges
        chmod 666 /var/run/docker.sock 2>/dev/null || \
            echo "ERROR: Could not modify Docker socket permissions. Service restart functionality may not work."
    fi
fi
